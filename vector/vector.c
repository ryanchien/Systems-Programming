...
vector *vector_create(copy_constructor_type copy_constructor,
                      ...
                      default_constructor_type default_constructor) {
	// your code here
	vector * v = malloc(sizeof(vector)); //119
	assert(v);
	v->copy_constructor = copy_constructor!=NULL ? copy_constructor : shallow_copy_constructor;
	v->destructor = destructor!=NULL ? destructor : shallow_destructor;
	v->default_constructor = default_constructor!=NULL ? default_constructor : shallow_default_constructor;
	v->size = 0;
	v->capacity = INITIAL_CAPACITY;
	v->array = malloc(v->capacity * sizeof(void*)); //117
...
void vector_resize(vector *this, size_t n) {
	...
	if (n < this->size) {
		...
		for (size_t i=this->size; i<n; i++) {
			((this->array)[i]) = (this->default_constructor()); //was calling constructor wrong
		}
	}
	this->size = n;
}
...
void vector_reserve(vector *this, size_t n) {
	...
	if (n > this->capacity) {
		...
		for (size_t i=old_cap; i<this->capacity; i++) {
			(this->array)[i] = NULL;
		}
	}
}
...
void vector_push_back(vector *this, void *element) {
    assert(this);
    // your code here
	vector_reserve(this, this->size+1); //changed from resize to reserve
	this->size++;
...
void vector_insert(vector *this, size_t position, void *element) {
	...
	if (!(position < this->size))
		printf("pos: %lu, size: %lu\n", position, this->size);
	assert(position < this->size);
	vector_reserve(this, this->size+1); //changed from resize to reserve
	this->size++;
	//reallocate elements after position
